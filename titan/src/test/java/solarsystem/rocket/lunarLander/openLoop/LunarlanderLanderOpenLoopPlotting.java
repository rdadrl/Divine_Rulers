...
public class LunarlanderLanderOpenLoopPlotting {
    public int iteration;

    @Test
    public void landerTest() {
        Date date = new Date(2000, 0, 1, 0, 0, 0);
        LunarlanderLanderOpenLoopVerlet rocket = new LunarlanderLanderOpenLoopVerlet( new Vector3D(1000, 170000
                , 0),
        ...
        // Create Chart
        XYChart chart = QuickChart.getChart("XY", "X", "Y", "XY", xData, yData);

        // Show it
        new SwingWrapper(chart).displayChart();

        try{
            BitmapEncoder.saveBitmap(chart, "./noWindXY", BitmapEncoder.BitmapFormat.PNG);
        }
        catch(IOException e){
