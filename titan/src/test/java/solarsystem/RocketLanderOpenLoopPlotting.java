...
public class RocketLanderOpenLoopPlotting {
    public int iteration;

    @Test
    public void landerTest() {
        Date date = new Date(2000, 0, 1, 0, 0, 0);
        RocketLanderOpenLoopVerlet rocket = new RocketLanderOpenLoopVerlet( new Vector3D(1000, 170000
                , 0),
        ...
        for (int i=0; i < (2000 * (1d / 0.01)); i++) {
            ...
            yData.add(rocket.centralPos.getZ());
            if (rocket.landed){
        ...
        // Create Chart
        XYChart chart = QuickChart.getChart("Z position", "Time", "Z position", "Z(t)", xData, yData);

        // Show it
        new SwingWrapper(chart).displayChart();

        try{
            BitmapEncoder.saveBitmap(chart, "./Zpos", BitmapEncoder.BitmapFormat.PNG);
        }
        catch(IOException e){
