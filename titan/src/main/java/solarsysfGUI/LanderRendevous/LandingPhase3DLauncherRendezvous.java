...
public class LandingPhase3DLauncherRendezvous extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        try {
            ...
            Vector3D initialPos = new Vector3D(0,0,0); //new Vector3D(-0.7071, 0.7071,0);
            //initialPos = initialPos.scale(0.5*sphereOfInfluence);
            //initialPos = initialPos.add((new Vector3D(0,-1,0)).scale(solarSystem.getPlanets().getTitan().getRadius()*1e03));
            Vector3D initialVel = new Vector3D(0,0,0).scale(5e02);
